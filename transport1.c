...
void transport_init(mysocket_t sd, bool_t is_active)
{
	...
	ctx->done == FALSE;
	ctx->recv_win = WINDOW_SIZE;
	ctx->send_win = STCP_MSS;
		...
		if (send_len > 0) {
			...
			if (recv_len >= sizeof(struct tcphdr)) {
				if (SYN_ACK->th_flags == (TH_ACK | TH_SYN)) {
					...
					ctx->adv_recv_win = ntohs(SYN_ACK->th_win);
					ctx->send_win = ntohs(MIN(ctx->recv_win, ctx->cong_win));
					ctx->send_base = ntohl(ctx->seq_num);
				...
				if (send_len > 0) {
					...
					if (len >= sizeof(struct tcphdr)) {
						if (ACK->th_flags == TH_ACK) {
							ctx->send_base = ntohl(ACK->th_ack);
...
static void control_loop(mysocket_t sd, context_t *ctx)
{
    ...
    while (!ctx->done)
    {
        ...
        if (event & APP_DATA)
        {
            ...
            /* see stcp_app_recv() */
			payload_size = MIN(STCP_MSS, Min(ctx->adv_recv_win,(ctx->send_win + ctx->send_base - ctx->seq_num)));
			if (payload_size > 0) {
				...
				if (app_size > 0) {
					...
					hdr->flags = TH_ACK;
					hdr->th_win = htons(ctx->send_win + (uint16_t)(ctx->send_base - ctx->seq_num)-payload_size);
		...
		if (event & NETWORK_DATA) {
			payload_size = MIN(STCP_MSS, ctx->recv_win + ctx->recv_base - ctx->seq_num);
			if (payload_size > 0){
