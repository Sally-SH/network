...
enum { CSTATE_ESTABLISHED, LISTEN, SYN_RCVD, SYN_SENT, FIN_WAIT_1, FIN_WAIT_2, CLOSE_WAIT, CLOSING, LAST_ACK, CLOSED }; /* obviously you should have more states */
...
void transport_init(mysocket_t sd, bool_t is_active)
{
	...
	if (is_active) {
		...
		SYN->th_flags = TH_SYN;
		SYN->th_win = htons(ctx->local_win);
		...
		if (send_len > 0) {
			...
			if (recv_len >= sizeof(struct tcphdr)) {
				if (SYN_ACK->th_flags == (TH_ACK | TH_SYN)) {
					...
					ACK->th_flags = TH_ACK;
					ACK->th_win = htons(ctx->local_win);
		...
		if (recv_len >= sizeof(struct tcphdr)) {
			if (SYN->th_flags == TH_SYN) {
				ctx->remote_win = ntohs(SYN->th_win);
				ctx->ack_num = ntohl(SYN->th_seq);
				ctx->seq_num = ntohl(SYN->th_ack);
				ctx->connection_state = SYN_RCVD;

				struct tcphdr *SYN_ACK = (struct tcphdr *)calloc(1, sizeof(struct tcphdr));
				SYN_ACK->th_seq = htonl(ctx->initial_sequence_num);
				SYN_ACK->th_ack = htonl(ctx->ack_num + 1);
      ...
      if (event & APP_DATA)
	{
	  ...
	  if (payload_size > 0) {
	    ...
	    if (app_size > 0) {
	      ...
	      if (send_len > 0) {
		printf("Send:\t%u\t%u\t%u\n",ctx->local_win,ctx->local_rem_win,payload_size);
				...
				if (recv_size >= sizeof(struct tcphdr)) {
					...
					if ((hdr->th_flags) & TH_ACK) {
						...
						if (data_size > 0 && (ctx->connection_state == CSTATE_ESTABLISHED)) {
							...
							ctx->ack_num = ntohl(hdr->th_seq) + data_size - 1;
							...
							ACK->th_off = htons(HEADER_SIZE);
							ACK->th_flags = TH_ACK;
							ACK->th_win = htons(ctx->local_rem_win);
							ssize_t len = stcp_network_send(sd, ACK, sizeof(struct tcphdr), NULL);
							if (len < 0) {
								errno = ECONNREFUSED;
							}
							...
							printf("Recv:\t%u\t%u\t%u\n",ctx->local_win,ctx->local_rem_win,ntohl(hdr->th_ack)-ctx->base);
							uint16_t cur_local_win = ctx->local_win;
							ctx->local_win = MIN(ctx->remote_win, ctx->cong_win);
							ctx->local_rem_win = ctx->local_win - cur_local_win + ctx->local_rem_win + ntohl(hdr->th_ack) - ctx->base;
